...
namespace GameCaro
{
    ...
    public class CaroChess
    {
        ...
        protected ChessPiece[,] arrChessPiece;
        protected ChessBoard chessBoard;
        protected Stack<ChessPiece> stkChessUsed;
        protected Stack<ChessPiece> stkChessUndo;
        ...
        public void CreateChessPieces()
        {
            for (int i = 0; i < chessBoard.NumOfLines; i++)
            {
                ...
                {
                    arrChessPiece[i, j] = new ChessPiece(i, j, new Point(j * ChessPiece._Width, i * ChessPiece._Height), 0); //tao ban co = mang 2 chieu 
                }
            }
        }
        // Phương thức đánh cờ: cha
        public bool PlayChess(int mouseX, int mouseY, Graphics g) //mouseX,Y la toa do cua form
        {
            if (mouseX % ChessPiece._Width == 0 || mouseY % ChessPiece._Height == 0) //check tro chuot click vao duong vien pbiet trai-phai
                return false; //thoat ra ngoai playchess dong thoi ko nhan gi ca
            int column = mouseX / ChessPiece._Width; //
            int row = mouseY / ChessPiece._Height;

            if (arrChessPiece[row, column].Owner != 0) //check vi tri con co cua player
                return false;
            switch (turn)
            {
                case 1:
                    ...
                    break;
                case 2:
                    arrChessPiece[row, column].Owner = 2;//player 2
            ...
            stkChessUndo = new Stack<ChessPiece>(); //tao 1 stack moi de undo
            ChessPiece tmp = arrChessPiece[row, column]; //lay quan co tu mang arrChessPiece 
            ChessPiece cp = new ChessPiece(tmp.Row, tmp.Column, tmp.Position, tmp.Owner);
            stkChessUsed.Push(cp);//gan ds con co da ton tai trong nuoc di tu cp de redo
        ...
        public virtual void StartGame(Graphics g)
        {
            ready = true; //bien danh dau dang choi de luu nc di vua danh vao stkChessUsed vs stkChessUndo
            stkChessUsed = new Stack<ChessPiece>(); //de redo
            stkChessUndo = new Stack<ChessPiece>(); //de undo
        }
        ...
        private ChessPiece FindMove()
        {
            ...
            long maxPoint = 0; //nuoc di toi uu
            for (int i = 0; i < chessBoard.NumOfLines; i++)
            {
                for (int j = 0; j < chessBoard.NumOfColumns; j++)
                {
